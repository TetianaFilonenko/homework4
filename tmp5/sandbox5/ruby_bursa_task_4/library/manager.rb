class Library::Manager
  ...
  def initialize readers = [], books = [], readers_with_books = []
    @readers, @books, @reader_with_books = readers, books, readers_with_books
    @statistics = {}
    populate_statistics!
  end

  def new_book author, title, price, pages_quantity, published_at
    @books << Library::PublishedBook.new author, title, price, pages_quantity, published_at
    @books.last
  end

  def new_reader  name, reading_speed
    @readers << Library::Reader.new name, reading_speed
    @readers.last
  end
  ...
  def give_book_to_reader reader_name, book_title
    ...
    @reader_with_books << Library::ReaderWithBook.new(book, reader)
