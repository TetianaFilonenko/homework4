...
describe Library::PublishedBook do
  let!(:published_book){Library::PublishedBook.new "", "", 1400, 340, 2015}

  it 'should be commentable' do
    published_book.add_comment "The comment"
    expect(Library::PublishedBook.comments_quantity).to eq 2
    expect(Library::Commentable.total_comments_quantity).to eq 3
    expect(published_book.comments.count).to eq 1
    expect(published_book.comments[0]).to eq "The comment"
  end

end

describe Library::ReaderWithBook do
  let!(:reader_with_book){Library::ReaderWithBook.new "", "", 0, 0}

  it 'should be commentable' do
    reader_with_book.add_comment "The comment"
    expect(Library::ReaderWithBook.comments_quantity).to eq 1
    expect(Library::Commentable.total_comments_quantity).to eq 4
    expect(reader_with_book.comments.count).to eq 1
  end
...
describe Library::Reader do
  ...
  it 'should be commentable' do
    expect(Library::Reader.group_identifier).to eq 1    
  end
